...
from WebMonitor.Database.database import WriteDatabase, read_data_db
import threading
import json
...
class ManageData(object):
    ...
    def create_data_flow(self):  # Create data to be sent to the database, and allow other threads to read from the db
        generate_dataThread = threading.Thread(target=self.db.store_data_db)  # Create thread handle
