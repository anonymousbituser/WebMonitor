...
def setup():  # Configure database
    ...
    try:
        ...
        cursor = connection.cursor()
        # Create string table for sensor data
        cursor.execute('CREATE TABLE IF NOT EXISTS hw_values (data text)')
        connection.commit()  # Push changes to database
        connection.close()
...
def write_database():  # Grabs data from MCU and stores it into specific table within the database
    ...
    while True:  # TODO: Pass flag thru thread
        ...
        data = get_dataset(tmp)
        cursor.execute("INSERT INTO hw_values (data) VALUES (?)", (data,))  # Store actual data
        connection.commit()  # Push changes to database
...
def read_data_db():  # Reads data from the database
    ...
    # cursor1.execute('SELECT * FROM fan_data ORDER BY rpm DESC LIMIT 2')
    # # cursor1.execute('SELECT rpm FROM fan_data WHERE ROWID=(?)', (cursor1.lastrowid,))
    ...
    cursor2.execute('SELECT * FROM hw_values ORDER BY data DESC LIMIT 1')
    # get_temperature = cursor2.fetchall()
    get_data = cursor2.fetchall()
    connectDB2.close()  # Close connection

    print("rpm value from db = ", get_data)
    return get_data
